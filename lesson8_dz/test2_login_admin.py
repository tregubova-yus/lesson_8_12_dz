...
# Страница логина в админку /admin/
# 1. Проверяем существующего пользователя
def test_login(browser, url, wait):
...
# 2. Проверяем логин/пароль несуществующего пользователя
def test_no_login(browser, url, wait):
    admin_login_page_url = url + f'/admin/'
    browser.get(admin_login_page_url)
    user_name = 'gggg'
    user_password = '5533'
...
# 3. Проверяем разлогинивание
def test_log_out(browser, url, wait):
    admin_login_page_url = url + f'/admin/'
    browser.get(admin_login_page_url)
    user_name = 'user'
    user_password = 'bitnami1'
